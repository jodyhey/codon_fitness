...
- Optional:
  ...
  - -y/--haploid-genotypes: genotype fields are single haploid GT values (0 or 1); count one allele per sample
  - -a: drop UPSTREAM, DOWNSTREAM, INTERGENIC, and UTR_* EFF entries before classifying sites
    ...
    for t2 in types2:
        for b1 in bases:
            for b2 in bases:
                ...
                #     continue
                t4 = snptype(b1, t2, b2)
                # Do not deduplicate here; canonical dedup happens later via key mapping.
                types4.append(t4)
...
def filter_eff_entries_legacy_like(eff_entries):
    """Drop noncoding-neighborhood SnpEff entries to mimic stripped EFF annotations."""
    filtered = []
    drop_exact = {'UPSTREAM', 'DOWNSTREAM', 'INTERGENIC'}
    for e in eff_entries:
        name = _eff_name(e)
        if name in drop_exact or name.startswith('UTR_'):
            continue
        filtered.append(e)
...
def first_called_gt_allele(gt):
    """Return the first called 0/1 allele from a pysam GT value, or None."""
    if gt is None:
        return None
    if isinstance(gt, int):
        return gt if gt in (0, 1) else None
    if len(gt) == 0:
        return None
    allele = gt[0]
...
def count_record_alleles(record, diploiddata=False, haploid_genotypes=False):
    """Count REF/ALT alleles from GT fields.

    Default behavior preserves the historical script behavior: use the first
    allele from each sample, with -p adding the second allele for diploid data.
    -y/--haploid-genotypes makes the intended one-allele-per-sample behavior
    explicit for VCFs whose GT fields are plain haploid values such as 0 or 1.
    """
    altcount = 0
    refcount = 0
    for sample in record.samples.values():
        ...
        if haploid_genotypes:
            allele = first_called_gt_allele(gt)
            if allele == 0:
                refcount += 1
            elif allele == 1:
                altcount += 1
        ...
        if diploiddata and gt is not None and not isinstance(gt, int) and len(gt) > 1:
            allele2 = gt[1]
            if allele2 == 0:
                refcount += 1
            elif allele2 == 1:
...
def run(args):
    ...
    # Deduplicate by canonical key so each SNP type gets one stable index.
    unique_snptypes = []
    seen_keys = set()
    for t in snptypes:
        k = snptype_key_str(t)
        if k in seen_keys:
            continue
        seen_keys.add(k)
        unique_snptypes.append(t)
    snptypes = unique_snptypes
    ...
    for record in vcf_in:
        ...
        if len(record.alleles[0]) != 1 or len(record.alleles[1]) != 1:
            continue
        if record.alleles[0] == record.alleles[1]:
            continue
        ...
        refcount, altcount = count_record_alleles(
            record,
            diploiddata=args.diploiddata,
            haploid_genotypes=args.haploid_genotypes
        )
        nc = altcount + refcount
        rikeys = record.info.keys()
        if 'EFF' in rikeys:
            rituple = record.info['EFF']
            if args.annotationfilter:
                rituple = filter_eff_entries_legacy_like(rituple)
            if len(rituple) == 0:
                continue
...
def parsecommandline():
    ...
    parser.add_argument("-p",dest="diploiddata",action="store_true", default = False,help=" genotypes are diploid, not haploid, use both")
    parser.add_argument("-y", "--haploid-genotypes", dest="haploid_genotypes", action="store_true", default=False,
                        help="GT fields are single haploid values such as 0 or 1, not diploid values like 0/0 or 0/1; count one called allele per sample")
    parser.add_argument("-g",dest="genomefafile",required = True,type=str,help="path to indexed genome reference file")
    parser.add_argument("-a",dest="annotationfilter",action="store_true", default = False,help="strip UPSTREAM, DOWNSTREAM, INTERGENIC, and UTR_* EFF entries before classifying sites")
    ...
    if args.diploiddata and args.haploid_genotypes:
        parser.error("use either -p/--diploid mode or -y/--haploid-genotypes, not both")
