#!/usr/bin/env python3
"""
Generate a square dotplot of y (col1) vs x (col2) from:
  D:\genemod\better_dNdS_models\popgen\Drosophila_SFS_and_SFRatios\manuscript\MBE\revision\figwork\Observed_codon_RSCU_and_g.txt

- X label: "Observed Codon RSCU"
- Y label: italic g
- Adds grid lines
...
Saves PNG to:
  D:\genemod\better_dNdS_models\popgen\Drosophila_SFS_and_SFRatios\manuscript\MBE\revision\figwork\Observed_codon_RSCU_and_g.png
"""
...
IN_WIN = r"D:\genemod\better_dNdS_models\popgen\Drosophila_SFS_and_SFRatios\manuscript\MBE\revision\figwork\Observed_codon_RSCU_and_g.txt"
OUT_WIN = r"D:\genemod\better_dNdS_models\popgen\Drosophila_SFS_and_SFRatios\manuscript\MBE\revision\figwork\Observed_codon_RSCU_and_g.png"
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            if len(parts) < 2:
                continue
            try:
                y = float(parts[0])  # first column is y
                x = float(parts[1])  # second column is x
                ys.append(y)
                xs.append(x)
            except ValueError:
...
def pearson_spearman_linreg(x: np.ndarray, y: np.ndarray):
    # Pearson
    if _HAVE_SCIPY:
...
def main():
    ...
    r2, slope, intercept, rho_s, p_s = pearson_spearman_linreg(x, y)
