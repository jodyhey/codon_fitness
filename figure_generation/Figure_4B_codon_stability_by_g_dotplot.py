...
Generate a square dotplot of y (col1) vs x (col2) from:
  D:\genemod\better_dNdS_models\popgen\Drosophila_SFS_and_SFRatios\manuscript\MBE\figwork\codon_stability_by_g.txt

- X label: italic g
- Y label: "Codon Stability Coefficient"
- Adds grid lines
- Computes linear regression, Pearson's R (and R^2), Spearman's rho and p-value
- Plots regression line

Saves PNG to:
  D:\genemod\better_dNdS_models\popgen\Drosophila_SFS_and_SFRatios\manuscript\MBE\figwork\codon_stability_byg.png
"""
...
IN_WIN = r"D:\\genemod\\better_dNdS_models\\popgen\\Drosophila_SFS_and_SFRatios\\manuscript\\MBE\\figwork\\codon_stability_by_g.txt"
OUT_WIN = r"D:\\genemod\\better_dNdS_models\\popgen\\Drosophila_SFS_and_SFRatios\\manuscript\\MBE\\figwork\\codon_stability_byg.png"
...
def main():
    ...
    # Labels
    ax.set_xlabel(r"$\hat{g}$", fontsize=18)
    ax.set_ylabel("Codon Stability Coefficient", fontsize=18)
    ax.tick_params(axis='both', which='both', labelsize=16)

    # Grid
    ax.grid(True, which='major', axis='both', linestyle='-', color='gray', linewidth=0.8, alpha=0.5)

    # Stats box (upper right to avoid overlap)
    text = (
        f"R$^2$ = {r2:.3f}\n"
        f"Slope = {slope:.3f}\n"
